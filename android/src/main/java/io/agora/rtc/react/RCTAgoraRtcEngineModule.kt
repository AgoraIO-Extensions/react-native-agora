...
class RCTAgoraRtcEngineModule(
    ...
    override fun destroy(callback: Promise?) {
        PromiseCallback(callback).resolve(engine()) { manager.destroy() }
    }
    ...
    override fun registerMediaMetadataObserver(callback: Promise?) {
        PromiseCallback(callback).code(manager.registerMediaMetadataObserver { methodName, data -> emit(methodName, data) })
    }

    @ReactMethod
    override fun unregisterMediaMetadataObserver(callback: Promise?) {
        PromiseCallback(callback).code(manager.unregisterMediaMetadataObserver())
    }

    @ReactMethod
    override fun setMaxMetadataSize(size: Int, callback: Promise?) {
        PromiseCallback(callback).code(manager.setMaxMetadataSize(size))
    }

    @ReactMethod
    override fun sendMetadata(metadata: String, callback: Promise?) {
        PromiseCallback(callback).code(manager.addMetadata(metadata))
    }
    ...
    override fun sendStreamMessage(streamId: Int, message: String, callback: Promise?) {
        PromiseCallback(callback).code(manager.sendStreamMessage(streamId, message))
    }
}
