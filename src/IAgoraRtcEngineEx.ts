...
export abstract class IRtcEngineEx extends IRtcEngine {
   ...
   * After initializing the video view of a remote user, you can call this method to update its rendering and mirror modes. This method affects only the video view that the local user sees.Call this method after rendering the RtcSurfaceView or RtcTextureView component corresponding to the remote user ID.During a call, you can call this method as many times as necessary to update the display mode of the video view of a remote user.
   *
   * @param uid The user ID of the remote user.
